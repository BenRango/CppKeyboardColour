...
void KeyboardCoulour::setAnimation()
{
    ...
    QString CKD_path = current_path + "\\CKD.exe";
    ...
    if (this->process->state() == QProcess::Running) {
        qDebug() << "Animation lancée avec succès : " << commande;
    } else {
        qDebug() << "Échec du lancement de l'animation.";
        qDebug() << "Erreur : " << this->process->errorString();
    }
}
